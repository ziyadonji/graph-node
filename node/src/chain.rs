...
/// Networks as chains will create the necessary chains fromt he adapter information.
/// There are two major cases that are handled currently:
/// Non substreams chains(Ethereum, Near, etc etc):
...
///  - The substreams adapters will trigger the creation of a Substreams chain, this should only be used for block ingestors as a last resort.
/// Substreams Chain:
